param(
    ...
    [ValidateSet(1, 2)][int]$NumLanes = 1,
    [double]$ClockPeriodNs = 10.5,
    [string]$OutputDir = "",
...
if ($MultiExercise) {
    ...
    $buildDir = Join-Path $repo "vivado_build\arty_s7_50_single_lanes1"
}
...
$tclScript = Join-Path $PSScriptRoot "vivado_build_arty_s7.tcl"
...
if (-not $SynthOnly) {
    if ($MultiExercise) {
        Write-Host "Bitstream (multi-date): $buildDir\arty_s7_qmc_multi.bit"
    } else {
        Write-Host "Bitstream (default): $buildDir\arty_s7_qmc.bit"
    }
} else {
