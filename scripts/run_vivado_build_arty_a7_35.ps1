...
Write-Host "Running Vivado batch for Arty A7-35T (repo: $repo) ..."
$proc = Start-Process -FilePath "vivado" -ArgumentList @(
    "-mode", "batch",
    "-source", (Join-Path $PSScriptRoot "vivado_build_arty_a7_35.tcl")
...
if (-not $SynthOnly) {
    Write-Host "Bitstream (default): $repo\vivado_build\arty_a7_35\arty_a7_qmc.bit"
}
